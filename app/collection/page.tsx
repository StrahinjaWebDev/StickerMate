  ...
  return (
    <div className="space-y-5">
      <section className="rounded-lg border border-line bg-white p-4 shadow-lift dark:border-white/10 dark:bg-neutral-900 sm:p-5">
        <h1 className="text-3xl font-black text-ink dark:text-white">{t("collection.title")}</h1>
      ...
      <section className="space-y-3 pb-4">
        ...
        <BulkActions />
        {viewMode === "list" ? (
          <StickerList list={stickers} query={query} filter={filter} heightClassName="h-[calc(100vh-260px)] min-h-[520px]" />
        ) : (
          <StickerGrid list={stickers} query={query} filter={filter} heightClassName="h-[calc(100vh-260px)] min-h-[560px]" />
        )}
      </section>
    </div>
  );
